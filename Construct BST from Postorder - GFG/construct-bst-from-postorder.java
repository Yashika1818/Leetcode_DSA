...
class GFG
{
    ...
    public static Node constructTree(int post[],int n)
    {
        ...
        idx=n-1;
        Node root=helper(post,n,lb);
        return root;
    }
    public static Node helper(int[] post,int n,int lb){
        if(idx<0 || post[idx]<lb){
            return null;
        }
        Node root=new Node(post[idx]);
        idx--;
        root.right=helper(post,n,root.data);
        root.left=helper(post,n,lb);
        return root;
    }
}